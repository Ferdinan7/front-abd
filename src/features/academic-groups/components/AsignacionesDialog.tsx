...
export function AsignacionesDialog({
  ...
  const {
    ...
    error: createError,
  } = useCreateAsignacion(grupo.id);
  const { mutate: deleteAsig, isPending: isDeleting } = useDeleteAsignacion(
    grupo.id,
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg bg-white max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Asigna materias y colaboradores a este grupo. Estas asignaciones se
            usan para generar el horario.
            ...
            <div className="space-y-1.5">
              {asignaciones.map((asig) => {
                ...
                const profesorNombre =
                  (asig.profesor as { nombre_completo?: string } | undefined)
                    ?.nombre_completo ?? asig.profesor_id;
