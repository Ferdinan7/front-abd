...
export function GroupDetailPage() {
  ...
  const { data: asignaciones, isLoading: loadingAsig } =
    useAsignaciones(groupId);
  const {
    mutate: createAsig,
    isPending: isCreating,
    error: createError,
  } = useCreateAsignacion(groupId);
  ...
  const noAsigWarn = generarResult?.warnings?.find(
    (w) =>
      w.materia === "N/A" || w.motivo?.toLowerCase().includes("asignacion"),
  );
  const realWarnings =
              ...
              <div className="divide-y divide-gray-50">
                {asignaciones.map((asig) => {
                  ...
                  const profesorNombre =
                    prof?.nombre_completo ?? asig.profesor_id;
            ...
            {!loadingHorario && !hasHorario && !isGenerating && (
              <div className="flex flex-col items-center justify-center py-16 text-center space-y-3">
                ...
                <p className="text-sm text-gray-400 max-w-xs leading-relaxed">
                  Asegúrate de tener asignaciones configuradas y luego presiona{" "}
                    ...
                    <tr>
                      ...
                      {DIAS.map((dia, di) => (
                        <th key={dia.key} className="p-2 min-w-[120px]">
                          <div
                            className={`rounded-lg border px-2 py-1.5 text-center text-[11px] font-bold uppercase tracking-wide text-gray-600 ${DIA_COLORS[di]}`}
                          >
                            {dia.label}
                          </div>
                        </th>
                        ...
                        <tr key={slot}>
                          ...
                          {DIAS.map((dia, di) => {
                            ...
                            return (
                              <td
                                key={dia.key}
                                className="p-1.5 align-top border-t border-gray-100"
                              >
                                {bloque ? (
                                  <div
                                    className={`h-full rounded-lg p-2 border ${DIA_COLORS[di]} space-y-1`}
                                  >
