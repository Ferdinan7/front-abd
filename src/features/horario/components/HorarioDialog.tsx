...
import { RefreshCw, Trash2, CalendarDays, AlertTriangle, Clock } from "lucide-react"
...
export function HorarioDialog({ open, onOpenChange, grupo }: HorarioDialogProps) {
    const { data: horario, isLoading, isError } = useHorario(grupo.id)
    const { mutate: generar, isPending: isGenerating, data: generarResult } = useGenerarHorario(grupo.id)
    const { mutate: eliminar, isPending: isDeleting } = useDeleteHorario(grupo.id)
            ...
            <DialogContent className="max-w-5xl w-full bg-white max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="text-xl font-extrabold text-gray-900 flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-gray-400 ml-1">· {turnoLabel}</span>
                    </DialogTitle>
                    <DialogDescription>
                        Horario semanal del grupo. Puedes generarlo automáticamente o eliminarlo.
                    </DialogDescription>
                </DialogHeader>

                {/* Warnings from last generation */}
                {generarResult?.warnings && generarResult.warnings.length > 0 && (() => {
                                ...
                                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-blue-600 mt-0.5">
                                            Este grupo no tiene materias ni profesores asignados. Abre el menú del grupo y selecciona <strong>Asignaciones</strong> para configurarlos antes de generar el horario.
                                        </p>
                                    </div>
                                </div>
                            )}
                ...
                {!isLoading && !hasHorario && !isGenerating && (
                    <div className="flex flex-col items-center justify-center py-12 text-center text-gray-400 space-y-2">
                        ...
                        <p className="text-sm">
                            Asegúrate de tener asignaciones configuradas antes de generar el horario.
                        </p>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-5 gap-3 pt-1 min-w-0">
                        {DIAS.map((dia, di) => (
                            <div key={dia.key} className="space-y-2">
                                <div className={`rounded-lg border px-3 py-2 text-center text-xs font-bold uppercase tracking-wide text-gray-600 ${DIA_COLORS[di]}`}>
                                    {dia.label}
                                </div>
                                <div className="space-y-1.5">
                                    {(horario[dia.key] ?? []).length === 0 ? (
                                        ...
                                        (horario[dia.key] ?? []).map((bloque, bi) => (
                                            ...
                                            >
                                                <div className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded ${BLOQUE_COLORS[bi % BLOQUE_COLORS.length]}`}>
                ...
                {isError && !isGenerating && (
                    <div className="flex flex-col items-center justify-center py-10 text-center text-gray-400 space-y-1">
                        ...
                        <p className="text-sm font-medium text-gray-500">Horario no generado aún</p>
                        <p className="text-xs">Usa el botón para crear el horario automáticamente.</p>
                    </div>
                )}
            </DialogContent>
        </Dialog>
