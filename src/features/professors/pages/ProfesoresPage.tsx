...
function NuevoProfesorDialog({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombre.trim() || !carreraId) return;
    mutate(
      {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-white">
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() =>
              mutate(profesor.id, { onSuccess: () => onOpenChange(false) })
            }
            disabled={isPending}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Registra los profesores. Ingresa su correo Google para que puedan
            acceder al sistema.
          </p>
        </div>
        <Button
        ...
        (profesores.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
            ...
            <p className="text-sm text-gray-400 max-w-xs leading-relaxed">
              Agrega los profesores de la carrera. Podrán iniciar sesión con su
              cuenta Google para ver su horario.
            </p>
            <Button
              onClick={() => setCreateOpen(true)}
              variant="outline"
